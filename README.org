...
*** Installing packages from source code
    There are several ways to do it, but I prefer [[https://github.com/quelpa/quelpa][Quelpa]] since it's compatible with Emacs [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Packages.html][Package manager]]

*** Definitions
    I put all defuns to separate packages/libraries to keep the config declarative
...
*** [[https://github.com/quelpa/quelpa][Quelpa]] "is a tool to compile and install Emacs Lisp packages locally from local or remote source code"
    ...
    #+END_SRC
    No more manual package installation, now we can install packages directly from github.
